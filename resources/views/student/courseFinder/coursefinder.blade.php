...
@push('header_script')
    ...
    <style>
        /* Force Montserrat globally in this view */
        .rounded-20 {
...
@section('content')
    ...
    <div class="filter-offcanvas" id="filterOffcanvas">
        ...
        <div class="offcanvas-body p-4 d-flex flex-column h-100">
            <form id="filterForm" method="GET" action="{{ route('student.courseFinder.filter') }}" class="d-flex flex-column h-100">
                <div class="flex-grow-1">
                    ...
                    <div class="mb-4">
                        ...
                        <select name="programLevelType" class="form-control custom-select-v2">
                            ...
                            <option value="Undergraduate" {{ request('programLevelType') == 'Undergraduate' ? 'selected' : '' }}>Undergraduate</option>
                            <option value="Postgraduate" {{ request('programLevelType') == 'Postgraduate' ? 'selected' : '' }}>Postgraduate</option>
                        </select>
                    </div>
                        ...
                        <div class="form-control custom-select-v2" style="height: 180px; overflow-y: auto; padding: 10px;">
                            @foreach ($universities as $university)
                                <div class="form-check mb-2">
                                    <input class="form-check-input" type="checkbox" name="universities[]" value="{{ $university->id }}" id="student_uni_{{ $university->id }}" {{ in_array($university->id, (array)request('universities', [])) ? 'checked' : '' }}>
                                    <label class="form-check-label text-dark" for="student_uni_{{ $university->id }}" style="cursor: pointer; font-size: 0.9rem;">
                    ...
                    <div class="mb-4">
                        ...
                        <div class="form-control custom-select-v2" style="height: 180px; overflow-y: auto; padding: 10px;">
                            @foreach ($states as $id => $name)
                                <div class="form-check mb-2">
                                    <input class="form-check-input" type="checkbox" name="state[]" value="{{ $id }}" id="student_state_{{ $id }}" {{ in_array($id, (array)request('state', [])) ? 'checked' : '' }}>
                                    <label class="form-check-label text-dark" for="student_state_{{ $id }}" style="cursor: pointer; font-size: 0.9rem;">
                                        {{ $name }}
                                    </label>
                                </div>
                            @endforeach
                    ...
                    <div class="mb-4">
                        <label class="small text-uppercase font-weight-bold text-muted mb-2 d-block">Program</label>
                        <select name="program" class="form-control custom-select-v2">
                            <option value="" disabled selected>Select Program</option>
                            @foreach ($courses as $type => $list)
                                @foreach ($list as $course)
                                    <option value="{{ $course->course_name }}" {{ request('program') == $course->course_name ? 'selected' : '' }}>{{ $course->course_name }}</option>
                                @endforeach
                            @endforeach
                        </select>
                    </div>
                    ...
                    <div class="mb-4">
                        <label class="small text-uppercase font-weight-bold text-muted mb-2 d-block">Intake</label>
                        <select name="intake" class="form-control custom-select-v2">
                            <option value="" disabled selected>Select Intake</option>
                            @foreach ([1 => 'Feb', 2 => 'July', 3 => 'Nov'] as $key => $label)
                                <option value="{{ $key }}" {{ request('intake') == $key ? 'selected' : '' }}>{{ $label }}</option>
                            @endforeach
                        </select>
                    </div>
                    ...
                    <div class="mb-4">
                        ...
                        <div class="form-control custom-select-v2" style="height: 180px; overflow-y: auto; padding: 10px;">
                            ...
                            @foreach ($studyAreas as $area)
                                <div class="form-check mb-2">
                                    <input class="form-check-input" type="checkbox" name="study_area[]" value="{{ $area }}" id="student_area_{{ $loop->index }}" {{ in_array($area, (array)request('study_area', [])) ? 'checked' : '' }}>
                                    <label class="form-check-label text-dark" for="student_area_{{ $loop->index }}" style="cursor: pointer; font-size: 0.9rem;">
                                        {{ $area }}
                                    </label>
                                </div>
                            @endforeach
    ...
    <div class="scholarship-bg-wrapper bg-alice-blue-custom">
        <header class="scholarship-hero" style="padding-top: 150px;">
            <div class="container">
            ...
            <div class="row">
                <div class="col-md-12">
                    <!-- Search Row V2 -->
                    <form method="GET" action="{{ route('student.courseFinder.filter') }}" class="w-100" id="searchFilterForm">
                        <div class="search-row-uni">
                            ...
                            <div class="filter-sort-group-v2">
                                <button type="button" class="btn-filter-pill-v2" id="filterToggleButton">Filter Courses <i class="fas fa-sliders-h ml-2"></i></button>

                                <div class="sort-by-capsule-v2 dropdown-toggle" data-bs-toggle="dropdown" aria-expanded="false" role="button">
                                    <span>Sort By: Popularity</span>
                                    <i class="fas fa-chevron-down small mb-0 ml-2"></i>
                                </div>
        ...
        <section class="scholarship-grid-section pt-5 pb-5">
            <div class="container container-wide">
                ...
                <div id="courseResults">
                    @include('student.courseFinder.course_filter', [
        ...
        $(document).ready(function() {
            ...
            function renderSuggestions(data) {
                ...
                data.forEach(item => {
                    ...
                    div.innerHTML = `
                        <i class="fas ${item.type === 'University' ? 'fa-university' : 'fa-book-open'}"></i>
